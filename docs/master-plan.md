...
14. **[🟡 partial]** Cursor pagination on the API (cards, users, comments). _(DONE: shared `apps/api/src/utils/cursorPagination.js` + `{ items, nextCursor }` on `/cards/feed`, `/cards/explore`, `/cards/:id/comments`, `/users/browse`, `/users/:id/followers`, `/users/:id/following`, `/users/suggested`; frontend `useCursorPagination` hook + reusable `InfiniteScroll` component drive infinite scroll on feed, profile grids, followers/following, likes modal, comments, and the notifications panel. See backlog "Infinite scroll" epic, phases 1–4, all merged.)_
    - **[✅ done] Chat CONVERSATION-list pagination** — branch `autopilot/2026-07-11` (awaiting review). GET /chats keyset-paginated by `updatedAt` → `{conversations, nextCursor, totalUnread}`; `cursorPagination` util extended with a configurable sort field (default createdAt, all existing callers unchanged). `totalUnread` moved SERVER-SIDE (computed over all conversations on the first page) so the nav badge no longer regresses when the client holds just one page; ChatProvider seeds it and keeps it live via socket deltas. Both the ChatPage list and the dock wired to InfiniteScroll. Chat MESSAGE list was already keyset (`c02502e`).
    - **[✅ done] Admin panels — server-side pagination** — branch `autopilot/2026-07-11` (awaiting review). New admin-guarded `GET /users/admin` + `GET /cards/admin` return `{items, total, page, limit}` via a MongoDB aggregation ($facet: items + count) with server-side search/filter/sort (escaped regex); users carry aggregated followersCount+postsCount, cards carry creatorName+likesCount. New `useOffsetPagination` hook drives the numbered-page UI; creator filter is now a server-side name search (scales past a dropdown).
    - REMAINING: `/cards/search` and `/users/search` (the all-posts / all-users browse pages) stay deliberately **offset-paginated**, not keyset — fine for now, revisit if they get hot. All app lists now paginate server-side.
...
15. **[🟡 partial]** React Query migration: feed becomes `useInfiniteQuery` **"load more" (20–30/page)** → kills the load-everything providers · follow/like/comment become optimistic mutations (no refresh, no scroll jump — the *real* fix) · admin analytics moves to server aggregation endpoints. _(DONE — the optimistic-mutation goal: FOLLOW was already in-place; LIKE (post) + comment-like are now optimistic with rollback in `CardsProvider` — branch `autopilot/2026-07-11`, browser-verified: instant flip, single PATCH, no refetch/scroll-jump. React Query itself deliberately NOT adopted yet — its optimistic cache only helps once the READS migrate off the Context providers, which is #16 + retiring getAllUsers/getAllCards; doing it now would collide with the fresh pagination. STILL OPEN: feed `useInfiniteQuery`/React Query adoption, retiring the load-everything providers, admin analytics → server aggregation. See the backlog "Infinite scroll" epic for the full provider-retirement blocker list + plan.)_
