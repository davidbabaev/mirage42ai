...
14. **[—not started]** Cursor pagination on the API (cards, users, comments). _(`cardsSvc.js` uses a flat `.limit(30)`, no cursor/skip.)_
    - **Server-authoritative follower/following counts** land with this. Both counts are still derived client-side from the full `users` array (`getFollowersCount` filters everyone; following is `new Set(user.following).size`) — which only works because the client loads every user. Cursor pagination kills that assumption, so the API must return the counts: `followingCount` as the deduped `$size` of the array, `followersCount` via `countDocuments({ following: id })` aggregation (or a maintained counter). The *display* was already made distinct-safe and the write path made atomic ($addToSet/$pull, ban cleanup, one-off dedupe migration 003) — commit `1d05612`; moving the count *source* to the server is the remaining piece.
