...
- Built on branch autopilot/2026-07-13, commit <pending>. The logged-in user object carried no counts, so the OWN sidebar/dashboard fell back to deriving "N posts" from the global cards array. New `attachOwnCounts()` in usersSvc attaches `postsCount` + `followersCount` + `followingCount` at the three auth entry points: register, login, and POST /auth/refresh. Same query semantics as GET /users/:id, so the numbers agree across surfaces.
