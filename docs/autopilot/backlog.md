...
- Still TODO (need their own order): retire the global getAllCards/getAllUsers providers so nothing loads the full collection (server-authoritative follower/following counts now shipped — commit 0d566b8 — removing one blocker to retiring the users provider). The admin panels no longer drive off those providers as of phase 7.
