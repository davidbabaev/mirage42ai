...
- Still TODO (need their own order): all-users page + all-posts/explore page — their client-side SEARCH + SORT + multi-filters must move server-side before they can paginate without breaking those filters; comments list — embedded array + optimistic add/like/reply reconciliation needs a careful data-flow refactor; chat message list (reverse/load-older scroll) + conversation list; admin panels (server-side paging); notifications (backend hard-caps at 50, needs a cursor param). The /cards/:id/comments endpoint already exists for when comments are wired.
