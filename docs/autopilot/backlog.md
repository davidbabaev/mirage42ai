...
- Mobile bug-fix batch (follow-up commit): (1) Posts page (`AllCardsPage.jsx`) — the posts column `<Grid size={{md:8}}>` was MISSING an `xs`, so on mobile MUI Grid2 auto-placed it to a 1-col span; that corrupted the CardItem full-bleed `calc(50% - 50vw)` math, shoving the list right with a gap. Fixed → `size={{xs:12, md:8}}` (0px overflow, flush). (2) Post modal close button (`CardPopupModal.jsx`) — un-tappable on mobile because react-zoom-pan-pinch's `transform` stacking context + `touchAction:'none'` swallowed the tap; added `zIndex:1102` to the close IconButton so the tap/hit-test reaches it (verified: tap closes at 390 AND 1280). (3) Desktop "People You May Know" right column (`FeedPage.jsx`) now `display={{xs:'none', md:'block'}}` — hidden on mobile (which has the inline MobileSuggestions carousel), shown on desktop. All browser-verified with a real mobile touch context (hasTouch/isMobile + touchscreen.tap).
- Known separate quirk (NOT fixed — out of scope, pre-existing): opening a post modal via a `?card=` deep-link URL on `/allcards` doesn't close on the X because a sync effect re-applies the param. Tap-to-open (the normal path) closes fine. Flagging for a future order.
