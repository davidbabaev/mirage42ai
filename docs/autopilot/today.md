...
### T1 — Onboarding/feed backend: fields, suggested users, popular-feed fallback
- What: Add `User.onboardingComplete: Boolean` (default false) and `User.interests: [String]` (default []). Add `PATCH /users/me/onboarding` (body: `{ interests?, onboardingComplete? }`, mutates only the caller). Add `GET /users/suggested?limit=&cursor=` returning real users to follow — exclude self, already-following, and block (either direction); order friends-of-friends first then by follower count; include `isFollowing` + minimal profile (name, job, avatar, followersCount). Update `cardsSvc.getFeedCards` so that when the viewer's visible-following set is empty, it falls back to popular public active posts (most-liked, then most-recent), block-aware, capped/paginated, and tags the response so the client can label it "Suggested for you" (e.g. return a clear `isSuggested` flag — keep the existing shape working).
- Decisions: interests options = the existing card category list (reuse, don't invent). Suggested limit default 10, cursor-based to match existing pagination. Popular fallback uses `status:'active'` only, excludes the viewer's own posts and blocked-either-way. Keep `getFeedCards` backward-compatible for users who DO follow people.
- Done when: tests green — new user (0 follows) gets popular posts from `/cards/feed` flagged suggested; `GET /users/suggested` excludes self/followed/blocked and returns isFollowing; `PATCH /users/me/onboarding` sets interests + onboardingComplete on the caller only; existing feed tests still pass.
- Type: logic
